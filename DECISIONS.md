...
- 2026-07-09: Use `infra/otel-demo/.env.demo` as the runtime env overlay for the OTel demo stack, because the upstream submodule does not ship a committed `.env` file and the compose stack needs explicit image, port, host, and collector path values to start reproducibly from this checkout.
- 2026-07-09: Remove `--feature-gates=service.profilesSupport` from the collector overlay, because the selected collector image rejects that gate and the flag is not required for the Phase 0 demo loop.
